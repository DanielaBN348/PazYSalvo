...
namespace PazYSalvoAPP.Business.Services
{
    public class PersonaService : IPersonaService
    {
        ...
        public async Task<Persona> Leer(int id)
        {
            ...
            Persona persona = await _context.Personas.FirstOrDefaultAsync(f => f.Id == id);  // Buscar la persona por su ID

            if (persona == null) return null; // Si la perosna no se encontró, devolver null

            return persona; // Devolver la persona encontrada
        }

        public async Task<IQueryable<Persona>> LeerTodos()
        {
            IQueryable<Persona> listaDePersonas = _context.Personas; // Obtener todas las personas del contexto

            return listaDePersonas; // Devolver la lista de personas
        }
    }
}
